...
from rag_agent import process_query, process_pdf_query, query_temporary, query_permanent
from deepfake_detector import detect_deepfake
from pdf_assistant import process_pdf
...
def pdf_query():
    ...
    try:
        ...
        if storage_type == 'temporary':
            response = query_temporary(query)
        else:
            response = query_permanent(query)
        return jsonify({'response': response})
    except Exception as e:
...
def set_storage(store_type):
    ...
    try:
        from pdf_assistant import save_pdf_to_vector_db
